...
static float filter(float altitude)
{
	...
	uint8_t j = index++ % SAMPLES;

	values[j] = altitude;
	float avgAltitude = values[j];

	for (uint8_t i = 1; i < SAMPLES; i++)
	{
		int8_t k = (j + i) % SAMPLES;
		int8_t pk = (0 == k) ? SAMPLES - 1 : k - 1;

		avgAltitude += values[pk] + LOW_PASS_FACTOR * (values[k] - values[pk]);
	}

	if (initCnt >= SAMPLES)
	{
		avgAltitude = avgAltitude / (float) (SAMPLES);
	}
	else
	{
		initCnt++;
